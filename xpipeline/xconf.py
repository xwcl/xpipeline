...
class Template:
    num : int = field()
    path : str = field(help='path to a PSF template')

@dataclass
class OtherTemplate(Template):
    size : int = field()

class AlignedCutouts(Command):
    """Cut out and regrid images
    """
    @dataclass
    class Configuration:
...
if __name__ == "__main__":
    ...
    class DemoCommand(Command):
        ...
        either_one : typing.Union[int, str] = field()
        should_bar : bool = field()
        should_foo : bool = field(help="Whether demo should foo")

    import coloredlogs
    coloredlogs.install(level='DEBUG', logger=log)
    d = Dispatcher([DemoCommand])
    d.main()
