    ...
    def main(self):
        ...
        if "vapp" in obs_method:
            ...
            if "SCI" not in template_hdul and len(template_hdul[0].data.shape) == 0:
                raise RuntimeError(
                    f"No 'SCI' extension in {self.args.template_psf} and no data in primary extension"
                )
            ...
            klip_inputs[0].sci_arr = characterization.inject_signals(
                klip_inputs[0].sci_arr, derotation_angles, specs, template_psf
            )
            fits.PrimaryHDU(klip_inputs[0].sci_arr.compute()).writeto(
                "./injected_cube.fits", overwrite=True
            )
