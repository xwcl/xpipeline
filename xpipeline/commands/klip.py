...
from ..tasks import (
    ...
    improc,
    vapp,
)  # obs_table, iofits, sky_model, detector, data_quality
...
class KLIP(MultiInputCommand):
    ...
    def _make_masks(self, sci_arr, extname):
        ...
        if self.args.mask_saturated:
            percentile_image = np.nanpercentile(
                sci_arr, self.args.mask_saturated_percentile, axis=0
            )
    ...
    def _assemble_out_image(self, obs_method, outcubes, derotation_angles):
        if "vapp" in obs_method:
            ...
            out_image = pipelines.vapp_stitch(
                left_cube, right_cube, self.args.vapp_symmetry_angle
            )
        else:
            out_image = pipelines.adi(
                outcubes[0], derotation_angles, operation=self.args.combine_by
            )
