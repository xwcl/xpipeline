...
class CollectDataset(MultiInputCommand):
    ...
    def main(self):
        ...
        obs_method = utils.parse_obs_method(' '.join(self.args.obs_method)) if self.args.obs_method is not None else {}
        extension_keys = self._normalize_extension_keys(self.args.ext) if self.args.ext is not None else []
        if self.args.vapp_left_ext is not None or self.args.vapp_right_ext is not None:
            if self.args.vapp_left_ext is None:
                log.error(f"Got --vapp-right-ext {self.args.vapp_right_ext} but no --vapp-left-ext")
                sys.exit(1)
            if self.args.vapp_right_ext is None:
                log.error(f"Got --vapp-left-ext {self.args.vapp_left_ext} but no --vapp-right-ext")
                sys.exit(1)
        ...
        static_header, metadata_table, cubes = dask.compute(static_header, metadata_table, cubes)
        ...
        if angle_keyword is not None:
            derotation_angles = angle_scale * metadata_table[angle_keyword] + angle_constant
            metadata_table = np.lib.recfunctions.append_fields(
                metadata_table,
                'derotation_angle_deg',
                derotation_angles
            )
            if 'ADI' not in obs_method:
                obs_method['adi'] = {}
            obs_method['adi']['derotation_angles'] = f'{obs_table_name}.derotation_angle_deg'
        ...
        for optflag, info in CollectDataset._keyword_override_options.items():
            ...
            if optval is None:
                if info['keyword'] not in static_header:
                    log.warn(f"No '{info['keyword']}' header found and no {optflag} option provided")
            else:
                if info['keyword'] in static_header:
                    log.warn(f"Both '{info['keyword']}' header ({static_header[info['keyword']]}) and {optflag} option ({optval}) provided, using option value")
                static_header[info['keyword']] = optval

        hdul = iofits.DaskHDUList([iofits.DaskHDU(data=None, header=static_header, kind='primary')])

        # handle paired vAPP cubes if requested
        if left_extname is not None:
            log.debug(f'Before crop: {cubes[left_extname].shape=} {cubes[right_extname].shape=}')
            cubes[left_extname], cubes[right_extname] = vapp.crop_paired_cubes(cubes[left_extname], cubes[right_extname])
            log.debug(f'After crop: {cubes[left_extname].shape=} {cubes[right_extname].shape=}')
