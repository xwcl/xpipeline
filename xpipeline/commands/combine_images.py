...
class CombineImages(MultiInputCommand):
    ...
    def main(self):
        ...
        for idx, extension in enumerate(hdul):
            ...
            if len(extension.data.shape) != expected_dim:
                continue
        ...
        if len(all_inputs) > 1:
            ...
            for extkey, extshape in exts.items():
                output_hdu = pipelines.combine_extension_to_new_hdu(coll, self.operation, extkey, extshape)
            ...
            for extname in exts:
                if hdul[extname].kind != 'image' or len(hdul[extname].data.shape) == 0:
                    continue
                new_data = pipelines.combine_cube(hdul[extname].data, operation=self.operation)
                outhdus.append(hdul[extname].updated_copy(new_data, history=f"Combined cube using {self.operation}"))
